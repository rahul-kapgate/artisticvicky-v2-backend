...
export const addVideo = async (req, res) => {
  try {
    ...
    if (!section_id || isNaN(Number(section_id))) {
      return res
        .status(400)
        .json({ success: false, message: "A valid section_id is required." });
    }
...
export const getVideos = async (req, res) => {
  try {
    ...
    let query = supabase
      .from("video_lectures")
      .select(
        "id, title, description, youtube_url, thumbnail_url, duration, is_free, section_id, created_at",
      )
...
export const getVideoById = async (req, res) => {
  try {
    ...
    if (!id || isNaN(Number(id))) {
      return res
        .status(400)
        .json({ success: false, message: "A valid video ID is required." });
    }
